"""
Async Cogniac Network Camera Object Client
...
mutable_keys = ['url', 'current_IP', 'camera_name', 'description',
                'active', 'lat', 'lon', 'hae', 'alt_subject_uid',
                'custom_configuration'] + camera_model_keys

immutable_keys = ['network_camera_id', 'created_at',
                  'created_by', 'modified_at', 'modified_by']


class AsyncCogniacNetworkCamera(object):
    """
    AsyncCogniacNetworkCamera
    Async version of CogniacNetworkCamera.
    ...
    async def create(cls,
        ...
        Plus optional device dictionaries returned from GVCP discovery.
        """
        ...
        resp = await connection._post("/1/networkCameras", json=data)
        return AsyncCogniacNetworkCamera(connection, resp.json())
    ...
    @classmethod
    @retry(stop=stop_after_attempt(8), wait=wait_exponential(multiplier=0.5), retry=retry_if_exception(server_error))
    async def get(cls, connection, network_camera_id):
        """
        Get a single network camera.

        connection (AsyncCogniacConnection): Authenticated AsyncCogniacConnection object
        network_camera_id (String):          the netcam id of the Cogniac NetCam object

        Returns AsyncCogniacNetworkCamera object.
        """
        resp = await connection._get("/1/networkCameras/%s" % network_camera_id)
        return AsyncCogniacNetworkCamera(connection, resp.json())
    ...
    async def get_all(cls, connection):
        """
        Return all AsyncCogniacNetworkCamera objects belonging to the authenticated tenant.

        connection (AsyncCogniacConnection): Authenticated AsyncCogniacConnection object
        """
        resp = await connection._get('/1/tenants/%s/networkCameras' % connection.tenant_id)
        netcams = resp.json()['data']
        return [AsyncCogniacNetworkCamera(connection, netcam) for netcam in netcams]
    ...
    def __init__(self, connection, netcam_dict):
        """
        Create an AsyncCogniacNetworkCamera.

        This is not normally called directly by users, instead use:
        AsyncCogniacNetworkCamera.create()
        """
        self._cc = connection
    ...
    async def set(self, **kwargs):
        """
        Update mutable network camera attributes via a single POST call.

        Accepted keys: url, current_IP, camera_name, description, active,
            lat, lon, hae, alt_subject_uid, custom_configuration,
            and all camera_model_keys (resolution, focal length, pose, etc.)

        Example:
            await camera.set(camera_name="new name", active=True)
        """
        for key in kwargs:
        ...
        resp = await self._cc._post("/1/networkCameras/%s" % self.network_camera_id, json=kwargs)
    ...
    async def delete(self):
        """
        Delete the network camera.
        """
        await self._cc._delete("/1/networkCameras/%s" % self.network_camera_id)
